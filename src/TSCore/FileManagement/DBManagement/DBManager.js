...
export default class DBManager {
    ...
    //添加下载任务
    addForceSuccessDownloadTask(file) {
        if (this.sql == null)
            return;
        let dto = new HeadImageDBDto();
        dto.HeadImageUrl = file.ResourceUrl;
        dto.userId = file.Param;
        let querySql = SQLiteHelper.sqlStringFormat(SQLiteSql.addTask, dto);
        this.sql.ExecuteSQL(querySql);
    }
    //删除下载任务
    deleteForceSuccessDownloadTask(userId) {
        if (this.sql == null)
            return;
        let dto = new HeadImageDBDto();
        dto.userId = userId;
        let querySql = SQLiteHelper.sqlStringFormat(SQLiteSql.removeTask, dto);
        this.sql.ExecuteSQL(querySql);
    }
    getAllForceSuccessDownloadTask(callback) {
        if (this.sql == null)
            return;
        this.sql.ExecuteSQLWithT(SQLiteSql.getAllTask, (data) => {
