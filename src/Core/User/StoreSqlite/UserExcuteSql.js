...
export const InitIMTable = {
   ...
   //如果是群聊 那么setting就是消息提醒设置，单聊则是允许对方看朋友群等.
    "createRelationSettingTable":"CREATE TABLE IF NOT EXISTS RelationSetting (Id INTEGER PRIMARY KEY AUTOINCREMENT,RelationId varchar(255), setTop Boolean, disturb Boolean,saveContact Boolean,showNick Boolean)",
...
export const ExcuteIMSql = {
    ...
    "InsertRelationSetting":"insert into RelationSetting(RelationId,setTop,disturb,saveContact,showNick) values(?,?,?,?,?)",
    "GetRelationSettingByRelationId":"select * from RelationSetting where RelationId=?",
    "UpdateRelationSetting":"update RelationSetting set setTop=?,disturb=?,saveContact=?,showNick=? where RelationId=?"
}