...
export function storeRecMessage(message) {
    ...
    CHATFMDB.InsertMessageWithCondition(message, message.Data.Data.Sender)
}
...
export function InsertResource(messageId, localPath) {
    CHATFMDB.InsertResource(messageId, localPath);
}

export function DeleteResource(messageId, localPath) {
    CHATFMDB.DeleteResource(messageId, localPath);
}

//向消息列表中添加消息
export function addMessageToSendSqlite(message) {
    CHATFMDB.addSendMessage(message);
}

//删除消息列表中的message记录
export function popMessageInSendSqlite(messageId) {
    CHATFMDB.popMessageInSendMessageSqlite(messageId);
}

//获取所有消息列表中的消息记录
export function getAllCurrentSendMessage(callback) {
    return CHATFMDB.getAllCurrentSendMessages(callback)
}

//修改发送队列中的消息状态
export function updateSendMessageStatus(message) {
    CHATFMDB.UpdateSendMessageStatues(message)
}
//修改消息列表中的消息状态
...
//todo：想办法进行批量操作
CHATFMDB.InsertMessageWithCondition = function (message, client) {
            ...
            tx.executeSql(checkChatExist, [client], (tx, results) => {
                if (results.rows.length) {
                    ...
                    tx.executeSql(createTableSql, [], (tx, results) => {
                        ...
                        errorDB('创建新聊天对象表', err)
                    });
                }
            }, (err) => {
                errorDB('查询所有聊天对象', err)
            });
...
//更新发送消息队列消息的状态
CHATFMDB.UpdateSendMessageStatues = function (message) {
...
//添加发送失败的消息进数据库
CHATFMDB.addSendMessage = function (message) {
...
CHATFMDB.popMessageInSendMessageSqlite = function (messageId) {
    var db = SQLite.openDatabase({
        ...databaseObj
    }, () => {
        db.transaction((tx) => {
...
//获取所有发送失败的消息
CHATFMDB.getAllCurrentSendMessages = function (callback) {
    var db = SQLite.openDatabase({
        ...databaseObj
    }, () => {
        db.transaction((tx) => {
...
CHATFMDB.InsertResource = function (messageId, localPath) {
...
CHATFMDB.DeleteResource = function (messageId, localPath) {
...
function updateChat(content, time, client, tx) {
    ...
    tx.executeSql(updateSql, [], (tx, results) => {
        ...
        errorDB('为' + client + "在会话列表中更新了最新的聊天记录", err)
    });
}
