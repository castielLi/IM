    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
      ...
      if (imageURL) {
        ...
        if (pickedAsset.creationDate) {
          self.response[@"timestamp"] = [[RNImagePicker ISO8601DateFormatter] stringFromDate:pickedAsset.creationDate];
        }
      }
            ...
            if (error) {
              ...
              if (assetURL) {
                ...
                if (capturedAsset.creationDate) {
                  self.response[@"timestamp"] = [[RNImagePicker ISO8601DateFormatter] stringFromDate:capturedAsset.creationDate];
                }
              }
      ...
      if (storageOptions && [[storageOptions objectForKey:@"cameraRoll"] boolValue] == YES && self.picker.sourceType == UIImagePickerControllerSourceTypeCamera) {
        ...
        [self convertVideoQuailtyWithInputURL:videoURL outputURL:videoDestinationURL completeHandler:^(bool success ,AVAssetExportSession* session) {
          if(success){
            self.callback(@[self.response]);
          }else{
            self.callback(@[@{@"error": @"转换视频格式失败"}]);
            return;
          }
...
- (void) convertVideoQuailtyWithInputURL:(NSURL*)inputURL
                         ...
                         completeHandler:(void (^)(bool,AVAssetExportSession*))handler
{
